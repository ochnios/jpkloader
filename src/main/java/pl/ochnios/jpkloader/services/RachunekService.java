    ...
    public ServiceResponse<Rachunek> createRachunek(String numer, String nip, String kodWaluty) {
        ...
        if (current.isPresent()) {
            ...
            if (!current.get().getWaluta().getKod().equals(kodWaluty)) {
                return ServiceResponse.fail(String.format("Rachunek %s już istnieje z walutą %s", numer, kodWaluty));
            }
            ...
            if (waluta.isEmpty()) {
                return ServiceResponse.fail(String.format("Brak waluty o kodzie ", kodWaluty));
            }
